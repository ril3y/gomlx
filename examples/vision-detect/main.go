...
func main() {
	if len(os.Args) < 4 {
		...
		fmt.Fprintf(os.Stderr, "\nDetect all instances of an object in an image (bounding boxes).\n")
		fmt.Fprintf(os.Stderr, "Example: %s ./models/moondream2 ./testdata/nozzle.png \"nozzle\"\n", os.Args[0])
		os.Exit(1)
	}
	modelPath := os.Args[1]
	imagePath := os.Args[2]
	...
	fmt.Printf("Detecting objects: %q in %s\n", object, imagePath)

	result, err := model.Detect(ctx, gomlx.Image{Path: imagePath}, object)
	if err != nil {
