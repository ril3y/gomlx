...
// A single Gemma 2 transformer layer.
// Gemma 2 uses four norms per layer (input, post-attention, pre-feedforward, post-feedforward).
struct Gemma2Layer {
    // Norms (4 per layer)
    std::optional<mlx::core::array> input_layernorm_weight;
    std::optional<mlx::core::array> post_attention_layernorm_weight;
    std::optional<mlx::core::array> pre_feedforward_layernorm_weight;
    std::optional<mlx::core::array> post_feedforward_layernorm_weight;
...
class Gemma2TextModel : public BaseModel {
public:
    explicit Gemma2TextModel(const ModelConfig& config);
    ...
    std::vector<int> stop_token_ids() const override { return {1, 107}; }
...
private:
    ...
    // MLP sub-layer (uses GELU instead of SiLU)
    ...
    // Gemma 2 specific: logit soft-capping
    float final_logit_softcapping_ = 30.0f;
    ...
    // Transformer layers
    std::vector<Gemma2Layer> layers_;
